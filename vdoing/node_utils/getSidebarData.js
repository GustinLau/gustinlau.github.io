...
function createSidebarData(sourceDir, collapsable) {
  ...
  tocs.forEach(toc => { // toc是每个目录的绝对路径
    ...
    if (toc.substr(-6) === '_posts') { // 碎片化文章
      ...
      sidebarData[`/${path.basename(toc)}/`] = sidebarObj.sidebar.map(i => {
        return i instanceof Array ? i.filter(v => typeof v === 'string') : i
      })
...
function mapTocToSidebar(root, collapsable, prefix = '', flat = false) {
  let sidebar = new Map(); // 结构化文章侧边栏数据
  const files = fs.readdirSync(root); // 读取目录（文件和文件夹）,返回数组

  files.forEach(filename => {
    const file = path.resolve(root, filename); // 方法：将路径或路径片段的序列解析为绝对路径
    const stat = fs.statSync(file); // 文件信息
    if (filename === '.DS_Store' || filename === 'index.md') { // 过滤.DS_Store文件
      return
    }
    ...
    // flat = false
    if (isDir) {
      if (fs.existsSync(path.resolve(root, 'index.md'))) {
        const contentStr = fs.readFileSync(path.resolve(root, 'index.md'), 'utf8') // 读取md文件内容，返回字符串
        const {data} = matter(contentStr, {}) // 解析出front matter数据
        flat = data.flat === true
      }
    }
    ...
    if (!flat) {
      ...
      if (sidebar.has(order)) { // 判断序号是否已经存在
        log(chalk.yellow(`warning: 该文件 "${file}" 的序号在同一级别中重复出现，将会被覆盖`))
      }
    }
    if (isDir) { // 是文件夹目录
      if (flat) {
        mapTocToSidebar(file, collapsable, prefix + filename + '/', flat).sidebar.forEach(value => sidebar.set(value[0], value))
      } else {
        sidebar.set(order, {
          title,
      ...
      sidebar.set(order, item);  // [<序号>, <路径>, <标题>, <永久链接>, <?标题标签>]
    }
  })
  return {
    sidebar: Array.from(sidebar)
      .sort((a, b) => +a[0] - +b[0])
      .map(item => item[1])
