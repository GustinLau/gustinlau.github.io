...
function mapTocToSidebar(root, collapsable, prefix = '', flat = false) {
  ...
  files.forEach((filename) => {
    const file = path.resolve(root, filename) // 方法：将路径或路径片段的序列解析为绝对路径
    const stat = fs.statSync(file) // 文件信息
    if (filename === '.DS_Store' || filename === 'index.md') {
      // 过滤.DS_Store文件
      return
    }
    ...
    if (isDir) {
      // 是文件夹目录
      if (flat) {
        mapTocToSidebar(file, collapsable, prefix + filename + '/', flat).sidebar.forEach((value) =>
          sidebar.set(value[0], value)
        )
      } else {
        sidebar.set(order, {
          title,
