...
export default {
  ...
  created() {
    for (let i = 0, tagH = this.tags.length; i < tagH; i++) {
      this.tagStyleList.push(this.getTagStyle(this.tags[i].key))
    }
  },
  computed: {
    tags() {
  ...
  methods: {
    getTagStyle(txt) {
      ...
      let randomColor
      if (txt) {
        randomColor = tagBgColor[txt.split('').reduce((s, c) => s + c.charCodeAt(), 0) % tagBgColor.length]
      } else {
        randomColor = tagBgColor[Math.floor(Math.random() * tagBgColor.length)]
      }
